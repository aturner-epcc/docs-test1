...
-   [STAT](#stat) generate merged stack traces for parallel
    applications. Also has visualisation tools.
...
in two nodes. We would launch this in gdb4hpc by running:
...
> -   `help` -- Lists all gdb4hpc commands. You can run
>     `help COMMAND_NAME` to learn more about a specific command (*e.g.*
>     `help launch` will tell you about the launch command
...
will be the one with the jobname `bash`. In this example:
...
There is a known issue with \`valgrind4hpc\`: the compiler wrappers
(ftn, cc, CC) do not work while this module is loaded. To compile, you
will need to unload the module (<span class="title-ref">module unload
valgrind4hpc</span>), compile, and reload the module (<span
class="title-ref">module load valrgind4hpc</span>).
...
he memory profiling data will be output into a file called
...
arguments (this is not necessary of your executable takes no arguments).
...
executable called `my_exe` using 256 processes, you would run:
...
> 154296 ? 00:00:00 systemd 154297 ? 00:00:00 (sd-pam) 154302 ? 00:00:00
> sshd 154303 pts/8 00:00:00 bash 157150 pts/8 00:00:00 salloc 157152
> pts/8 00:00:00 bash 157183 pts/8 00:00:00 srun 157185 pts/8 00:00:00
> srun 157191 pts/8 00:00:00 ps
...
command (note that "my\_exe" will need to be replaced with the name of
the executable you ran):

    stat-view stat_results/my_exe.0000/00_my_exe.0000.3D.dot

This produces a graph displaying all the different places within the
program that the parallel processes were when you queried them.
